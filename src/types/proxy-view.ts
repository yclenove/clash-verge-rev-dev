...
/** Built-in policy names from mihomo ? not real outbound servers. */
const PRESET_PROXY_POLICY_NAMES = new Set([
  'DIRECT',
  'REJECT',
  'REJECT-DROP',
  'PASS',
...
const isPresetProxyPolicyName = (name: string) =>
  PRESET_PROXY_POLICY_NAMES.has(name)
...
export const selectGlobalChainNodes = (
  ...
  view.global === null
    ? []
    : selectRuntimeStandaloneNodes(view, runtimeProxies).filter(
        (node) => !isPresetProxyPolicyName(node.name),
      )
...
export const selectRuleChainMembers = (
  ...
  return group.members.flatMap((memberRef, memberIndex) => {
    ...
    if (member.kind === 'node' && isPresetProxyPolicyName(member.node.name)) {
      return []
    }
    return [{ memberIndex, member }]
  })
...
/**
 * All unique leaf nodes usable when building a proxy chain under multi-sub
 * always-on. Rule-mode "select group" only chooses the exit target group; the
 * chain itself may include nodes from any merged subscription.
 * Built-in policies (DIRECT/REJECT/...) are excluded ? they are not hop nodes.
 */
export const selectAllChainNodes = (view: ProxyViewV1): ProxyNodeView[] => {
  const byRecordId = new Map<string, ProxyNodeView>()

  const takeNode = (node: ProxyNodeView) => {
    if (isPresetProxyPolicyName(node.name)) return
    if (!byRecordId.has(node.recordId)) {
      byRecordId.set(node.recordId, node)
    }
  }
  ...
  for (const group of view.groups) {
    for (const memberRef of group.members) {
      const member = resolveMember(view, memberRef)
      if (member.kind === 'node') takeNode(member.node)
    }
  }

  if (view.global) {
    for (const memberRef of view.global.members) {
      const member = resolveMember(view, memberRef)
      if (member.kind === 'node') takeNode(member.node)
    }
  }

  for (const node of resolveStandaloneNodes(view)) {
    takeNode(node)
  }
