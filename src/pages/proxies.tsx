...
const resolveChainExitNode = (verge?: IVergeConfig | null): string | null => {
  try {
    const fromStorage = localStorage.getItem('proxy-chain-exit-node')
    if (fromStorage?.trim()) return fromStorage.trim()
  } catch {
    // ignore private mode / quota errors
  }

  const nodes = verge?.proxy_chain_nodes
  ...
  if (
    ...
    typeof last === 'object' &&
    'name' in last &&
    typeof (last as { name?: unknown }).name === 'string'
  ) {
    const name = (last as { name: string }).name.trim()
    return name || null
  }
  return null
}
...
const ProxyPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchChainConfig = async () => {
      try {
        const exitNode = resolveChainExitNode(verge)
        ...
        if (!localStorage.getItem('proxy-chain-exit-node')) {
          localStorage.setItem('proxy-chain-exit-node', exitNode)
        }
  ...
  }, [isChainMode, updateChainConfigData, verge])

  useEffect(() => {
    if (!normalizedMode || isMode(normalizedMode)) return
    void patchClashMode('rule').then(() => refreshClashConfig())
  }, [normalizedMode, refreshClashConfig])
