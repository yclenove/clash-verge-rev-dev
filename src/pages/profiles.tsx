...
const ProfilePage = () => {
  ...
  // Multi-sub merge is always on: every remote/local subscription is active.
  const currentActivatings = () => {
    return profileItems.map((item) => item.uid).filter(Boolean)
  }
  ...
  // Choose base profile only (proxy-groups / per-profile overrides).
  // All subscriptions stay active for node merge — not exclusive activation.
  ...
  const onDelete = useLockFn(async (uid: string) => {
    try {
      setActivatings([...new Set([...currentActivatings(), uid])])
      await deleteProfile(uid)
      mutateProfiles()
      mutateLogs()
      // Always rebuild runtime: every subscription contributes nodes.
      await onEnhance(false)
    } catch (err: any) {
  ...
  const deleteSelectedProfiles = useLockFn(async () => {
    ...
    try {
      const deletedUids = [...selectedProfiles]
      setActivatings((prev) => [
        ...new Set([...prev, ...currentActivatings(), ...deletedUids]),
      ])
      ...
      // Always rebuild: every subscription contributes nodes under multi-merge.
      await onEnhance(false)
  ...
  return (
    ...
    >
                ...
                {profileItems.map((item) => (
                  <Grid size={{ xs: 12, sm: 6, md: 4, lg: 3 }} key={item.file}>
                    <SortableProfileItem
                      id={item.uid}
                      selected={true}
                      isBase={item.uid === (switchTarget ?? profiles.current)}
                      activating={
                      ...
                      onSave={async (prev, curr) => {
                        // Any subscription content change can affect merged nodes.
                        if (prev !== curr) {
                          await onEnhance(false)
                        }
                      }}
      ...
      <ProfileViewer
        ref={viewerRef}
        onChange={async (shouldEnhance) => {
          mutateProfiles()
          // Multi-sub always-on: any remote/local change can affect merged nodes.
          if (shouldEnhance) {
            await onEnhance(false)
          }
        }}
