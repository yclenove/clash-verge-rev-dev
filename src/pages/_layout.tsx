...
const Layout = () => {
  ...
  const logAlertCount = useSyncExternalStore(
    subscribeLogAlert,
    getLogAlertUnreadCount,
    getLogAlertUnreadCount,
  )

  // Visiting Logs clears the badge; pause the background socket to avoid duplicates.
  // The Logs page feeds the rate window directly while it is open.
  useEffect(() => {
    const onLogs = location.pathname === '/logs'
    setLogsPageActive(onLogs)
    setLogAlertMonitorPaused(onLogs)
  }, [location.pathname])
                    ...
                    {menuOrder.map((path) => {
                      ...
                      return (
                        <SortableNavMenuItem
                          ...
                          item={item}
                          label={t(item.label)}
                          badgeCount={isLogs ? logAlertCount : 0}
                          onActivate={
                            isLogs ? () => clearLogAlertUnread() : undefined
                          }
                        />
                      )
                    })}
