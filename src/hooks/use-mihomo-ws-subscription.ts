  ...
  entry.connectWs = async () => {
    ...
    try {
      ...
      entry.staleTimer = setInterval(() => {
        ...
        if (Date.now() - entry.lastActivityAt > STALE_SOCKET_MS) {
      ...
      if (owner?.onConnected) {
        try {
          ...
          console.warn('[MihomoWs] onConnected failed', error)
        }
        if (entry.closed) {
          await ws.close()
          return
  ...
  useEffect(() => {
    ...
    const baseNext: NextFn<T> = (error, data) => {
      if (error !== undefined && error !== null) {
        console.warn('[MihomoWs] handler reported error', error)
        return
      }
