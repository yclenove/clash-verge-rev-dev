...
import { decodeGroupListKey, encodeGroupListKey } from '@/utils/group-list-key'
...
export const useGroupsDelays = (
  ...
  // JSON-encoded so group names that contain spaces are not split apart.
  const groupKey = encodeGroupListKey(groups)

  const subscribe = useCallback(
    (onSettle: () => void) => {
      const names = decodeGroupListKey(groupKey)
