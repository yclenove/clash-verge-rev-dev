...
export interface ProfileItemProps {
  selected: boolean
  /** base config source under multi-sub always-on */
  isBase?: boolean
  activating: boolean
  ...
  return (
    <Box sx={{ position: 'relative' }}>
      <ProfileBox
        ...
        onClick={(e) => {
          // Multi-sub always-on: card click no longer exclusive-activates.
          // Batch mode still toggles multi-select; otherwise open no activation path.
          if (activating) {
            e.preventDefault()
            e.stopPropagation()
            return
            ...
            >
              ...
              {isBase && (
                <Chip
                  ...
                  color="primary"
                  variant="outlined"
                  label={t('profiles.components.badges.baseConfig')}
                  sx={{ height: 20, flexShrink: 0, fontSize: 11 }}
                />
              )}
            </Box>
          </Box>
