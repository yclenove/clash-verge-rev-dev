...
interface Props {
  onChange: (shouldEnhance?: boolean) => void
}
  ...
  const handleOk = useLockFn(
    handleSubmit(async (form) => {
      ...
      try {
        ...
        // Multi-sub always-on: any remote/local create/edit affects merged nodes.
        const shouldEnhance = form.type === 'remote' || form.type === 'local'

        // Keep the full option object; patch replaces the whole `option` struct.
        const originalOption = { ...item.option }
        ...
        if (!isRemote) {
          ...
          try {
            ...
            if (openType === 'new') {
              ...
              // Restore the full option after a self-proxy retry in edit mode.
              await patchProfile(form.uid, { option: originalOption })
            }
