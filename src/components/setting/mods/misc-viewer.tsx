...
const clampVergeLatencyTimeout = (value: number) => {
  if (!Number.isFinite(value) || value < 1) return 10000
  return Math.min(VERGE_LATENCY_TIMEOUT_MAX, Math.floor(value))
}
  ...
  const onSave = useLockFn(async () => {
    try {
      await patchVerge({
        ...
        default_latency_timeout: clampVergeLatencyTimeout(
    ...
    >
      <List>
        ...
        <ListItem sx={{ padding: '5px 2px' }}>
          ...
          <TextField
            ...
            onChange={(e) =>
              setValues((v) => ({
                ...v,
                defaultLatencyTimeout: clampVergeLatencyTimeout(
        ...
        <ListItem sx={{ padding: '5px 2px' }}>
          ...
          />
          <TooltipIcon
            title={t('settings.modals.misc.tooltips.autoCheckUpdate')}
            sx={{ opacity: '0.7' }}
          />
          <Switch
            edge="end"
