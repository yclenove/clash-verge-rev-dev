...
export const ClashPortViewer = forwardRef<ClashPortViewerRef>((_, ref) => {
  ...
  const [enableRandomPort, setEnableRandomPort] = useState(
    verge?.enable_random_port ?? false,
  )
  ...
  useImperativeHandle(ref, () => ({
    open: () => {
      ...
      setEnableRandomPort(verge?.enable_random_port ?? false)
      setOpen(true)
    },
    close: () => setOpen(false),
  }))
  ...
  const onSave = useLockFn(async () => {
    ...
    const duplicatePort = portList.find(
      (port, index) => portList.indexOf(port) !== index,
    )
    ...
    // 验证端口范围
    const isValidPort = (port: number) => port >= 1000 && port <= 65535
    const allPortsValid = [
      mixedPort,
    ...
    if (!allPortsValid) {
      const invalidPort = [
        ...
        redirEnabled ? redirPort : 0,
        tproxyEnabled ? tproxyPort : 0,
      ].find((port) => port !== 0 && !isValidPort(port))
      showNotice.error(
        invalidPort != null && invalidPort < 1000
    ...
    try {
      await patchVerge({ enable_random_port: enableRandomPort })
    } catch (error) {
      showNotice.error('settings.modals.clashPort.messages.saveFailed', error)
      return
    }
    ...
    >
      <List sx={{ width: '100%' }}>
        <ListItem sx={{ padding: '4px 0', minHeight: 36 }}>
          <ListItemText
            primary={t('settings.modals.clashPort.fields.enableRandomPort')}
            secondary={t('settings.modals.clashPort.tooltips.enableRandomPort')}
            slotProps={{
          ...
          />
          <Switch
            checked={enableRandomPort}
            onChange={(_, checked) => setEnableRandomPort(checked)}
          />
        </ListItem>
        <ListItem sx={{ padding: '4px 0', minHeight: 36 }}>
          <ListItemText
