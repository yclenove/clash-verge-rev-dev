...
import { resolveClashLogDisplayPayload } from '@/utils/translate-clash-log'
...
/**
 * Mapping from raw core log levels to shared i18n label keys.
 * The core may emit both long and short forms (warning/warn, error/err).
 */
const LOG_LEVEL_LABEL_KEYS: Record<string, TranslationKey> = {
  debug: 'shared.filters.logLevels.debug',
  info: 'shared.filters.logLevels.info',
  inf: 'shared.filters.logLevels.info',
  warning: 'shared.filters.logLevels.warn',
  warn: 'shared.filters.logLevels.warn',
  error: 'shared.filters.logLevels.error',
  err: 'shared.filters.logLevels.error',
}

const LogItem = ({ value, searchState }: Props) => {
  const { t, i18n } = useTranslation()
  const displayPayload = resolveClashLogDisplayPayload(
    value.payload,
    i18n.language,
  )
  ...
  return (
    <Item>
      <div>
        ...
        <span className="type" data-type={(value.type || 'info').toLowerCase()}>
          {renderHighlightText(
            LOG_LEVEL_LABEL_KEYS[(value.type || 'info').toLowerCase()]
              ? t(LOG_LEVEL_LABEL_KEYS[(value.type || 'info').toLowerCase()])
              : value.type || 'info',
          )}
        </span>
      </div>
      <div>
        <span className="data">{renderHighlightText(displayPayload)}</span>
      </div>
    </Item>
  )
}
