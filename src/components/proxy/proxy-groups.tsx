...
import { ProxyGroupsChain } from './proxy-groups-chain'
...
function ChainProxyGroups(props: {
  ...
  const { proxyView } = useProxiesData()
  // Preferred rule target group for chain exit. Separate from
  // proxy-chain-group (active connected group), which is cleared on disconnect.
  const [selectedGroup, setSelectedGroup] = useState<string | null>(() => {
    try {
      return (
        localStorage.getItem('proxy-chain-rule-group') ||
        localStorage.getItem('proxy-chain-group')
      )
    } catch {
      return null
    }
  ...
  }, [availableGroups, defaultRuleGroup, mode, selectedGroup])

  // Keep localStorage aligned when the stored rule group is no longer valid.
  useEffect(() => {
    if (mode !== 'rule') return
    if (!activeSelectedGroup) return
    if (selectedGroup === activeSelectedGroup) return
    try {
      localStorage.setItem('proxy-chain-rule-group', activeSelectedGroup)
    } catch {
      // ignore quota / private mode errors
    }
    // eslint-disable-next-line @eslint-react/set-state-in-effect -- resync stale selection after group list changes
    setSelectedGroup(activeSelectedGroup)
  }, [activeSelectedGroup, mode, selectedGroup])

  const handleSelectRuleGroup = useCallback((groupName: string) => {
    setSelectedGroup(groupName)
    try {
      localStorage.setItem('proxy-chain-rule-group', groupName)
    } catch {
      // ignore quota / private mode errors
    }
  }, [])
  ...
  return (
    <ProxyGroupsChain
      ...
      onGroupSelect={handleSelectRuleGroup}
