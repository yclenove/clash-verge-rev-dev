...
import { ProxyChainSelectItem } from './proxy-chain-select-item'
...
export const ProxyRender = memo(function ProxyRender(props: RenderProps) {
  ...
  if (type === 0 && isChainMode) {
    // Subscription section header - match chain-picker visual: name + count.
    return (
      <Box sx={{ px: 1.5, pt: 1, pb: 0.25 }}>
        <ListItemButton
          ...
          sx={{
            ...
            px: 1,
            py: 0.5,
            bgcolor: alpha(isDark ? '#fff' : '#000', stickyed ? 0.06 : 0.03),
          }}
        >
          <ListItemText
            primary={
          ...
          <Box sx={{ width: '100%' }}>
            <Box sx={{ display: 'flex', alignItems: 'center', width: '100%' }}>
              ...
              {enable_group_icon && group.icon?.trim().startsWith('<svg') && (
                <img
                  src={`data:image/svg+xml,${encodeURIComponent(group.icon)}`}
                  alt="group icon"
                  width="32px"
                />
              )}
              <ListItemText
                ...
                secondary={
                  ...
                  >
                    ...
                    >
                      <StyledTypeBox>
                        {translateGroupType(group.type, t)}
                      </StyledTypeBox>
