...
import { downloadToTempAndReplace } from './prebuild-resource-utils.mjs'
...
// Node's global fetch (undici) ignores the legacy `agent` option, so GitHub
// downloads would bypass HTTP(S)_PROXY and time out in restricted networks.
// Use node:https/http + HttpsProxyAgent instead, with redirect following.
// =======================
const getHttpProxy = () =>
...
const proxyFetch = (url, init = {}, redirectCount = 0) =>
    ...
    const req = transport.request(
      ...
      (res) => {
        ...
        if (
          ...
          res.resume()
          proxyFetch(nextUrl, init, redirectCount + 1).then(resolve, reject)
          return
        }
...
async function getLatestAlphaVersion() {
  ...
  try {
    const response = await proxyFetch(META_ALPHA_VERSION_URL, {
      method: 'GET',
    ...
    // Fall back to stale cache so local builds can continue offline / when GitHub is flaky.
    const cache = await loadVersionCache()
    const stale = cache.META_ALPHA_VERSION?.version
    if (stale) {
      META_ALPHA_VERSION = stale
      log_error(
        'Error fetching latest alpha version:',
        err.message,
        `(using stale cache ${stale})`,
      )
      return
    }
...
async function getLatestReleaseVersion() {
  ...
  try {
    const response = await proxyFetch(META_VERSION_URL, {
      method: 'GET',
      headers: { 'User-Agent': 'clash-verge-rev-prebuild' },
    })
    if (!response.ok)
    ...
    // Fall back to stale cache so local builds can continue offline / when GitHub is flaky.
    const cache = await loadVersionCache()
    const stale = cache.META_VERSION?.version
    if (stale) {
      META_VERSION = stale
      log_error(
...
async function downloadFile(url, outPath) {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to download ${url}: status ${response.status}${snippet ? `, body="${snippet}"` : ''}`,
    )
  }
...
async function resolveResource(binInfo) {
  ...
  if (downloadURL) {
    ...
    await downloadToTempAndReplace(targetPath, (tempPath) =>
