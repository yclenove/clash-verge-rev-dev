...
fn locale_alias(locale: &str) -> Option<&'static str> {
    match locale {
        ...
        "zh-tw" | "zh-hk" | "zh-hant" | "zh-mo" | "zh-cht" => Some("zh"),
        _ => None,
    }
}
...
mod test {
    ...
    fn test_resolve_supported_language() {
        ...
        assert_eq!(resolve_supported_language("zh-Hant").as_deref(), Some("zh"));
        assert_eq!(resolve_supported_language("jp").as_deref(), Some("en"));
        assert_eq!(resolve_supported_language("ja-JP").as_deref(), Some("en"));
