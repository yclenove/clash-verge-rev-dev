...
mod app_init {
    ...
    pub fn setup_plugins(builder: tauri::Builder<tauri::Wry>) -> tauri::Builder<tauri::Wry> {
        #[cfg(target_env = "ohos")]
        {
            // crates.io shell/clipboard/notification assume android/ios under
            // cfg(mobile); OHOS uses local stubs. Mihomo plugin is real (API client).
            return builder
                .plugin(tauri_plugin_clash_verge_sysinfo::init())
                .plugin(crate::platform_plugins::notification::init())
                .plugin(crate::platform_plugins::clipboard_manager::init())
                .plugin(crate::platform_plugins::shell::init())
                .plugin(
    ...
    #[cfg(target_env = "ohos")]
    pub fn setup_deep_links(_app: &tauri::App) {}
...
pub fn run() {
    // reqwest uses rustls-no-provider so we avoid aws-lc (needs cmake) on OHOS.
