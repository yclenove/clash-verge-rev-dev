...
pub fn app_home_dir() -> Result<PathBuf> {
    ...
    #[cfg(target_env = "ohos")]
    {
        // HarmonyOS sandbox — do not use XDG/dirs (no HOME).
        return Ok(PathBuf::from("/data/storage/el2/base/files").join(APP_ID));
    }
...
pub fn preinit_app_data_dir() -> Result<PathBuf> {
    ...
    // Android sandbox path — OHOS uses /data/storage/el2/base instead (see app_home_dir).
    #[cfg(all(clash_verge_mobile, not(target_env = "ohos")))]
    let root = PathBuf::from("/data/data/io.github.clash-verge-rev.clash-verge-rev");
    #[cfg(target_env = "ohos")]
    let root = PathBuf::from("/data/storage/el2/base/files");
...
#[cfg(all(not(target_os = "macos"), not(clash_verge_mobile)))]
pub fn sidecar_ipc_path() -> Result<PathBuf> {
