...
pub async fn build_new_window() -> Result<WebviewWindow, String> {
    ...
    let prefers_dark_background = match resolved_theme {
        ...
        _ => {
            #[cfg(not(clash_verge_mobile))]
            {
                !matches!(detect_system_theme().ok(), Some(SystemTheme::Light))
            }
    ...
    // OHOS/Android wry does not reliably emit PageLoadEvent::Finished; starting
    // hidden + only showing on that event leaves a permanent black window.
    #[cfg(clash_verge_mobile)]
