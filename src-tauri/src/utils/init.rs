...
pub(super) async fn init_dns_config() -> Result<()> {
    ...
    let dns_config = serde_yaml_ng::Mapping::from_iter([
        ...
        ("listen".into(), Value::String(DEFAULT_DNS_LISTEN.into())),
    ...
    if !dns_path.exists() {
        ...
        let yaml = fs::read_to_string(&dns_path).await?;
        let mut existing = serde_yaml_ng::from_str::<serde_yaml_ng::Mapping>(&yaml)?;
        if crate::utils::dns_config::normalize_dns_listen(&mut existing) {
            help::save_yaml(&dns_path, &existing, Some("# Clash Verge DNS Config")).await?;
            logging!(
                warn,
                Type::Setup,
