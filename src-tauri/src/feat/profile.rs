...
/// Set the base proxy profile (proxy-groups / per-profile overrides).
/// Multi-sub merge keeps every remote/local subscription active for nodes.
...
async fn perform_profile_update(
    ...
    _is_mannual_trigger: bool,
) -> Result<bool> {
    logging!(info, Type::Config, "[订阅更新] 开始下载新的订阅内容");
    let mut merged_opt = PrfOption::merge(opt, option);
    // Multi-sub merge is always on: any subscription update can change runtime nodes,
    // so callers should refresh config after a successful update (not only current).
    let profiles = Config::profiles().await;
    ...
    match PrfItem::from_url(url, None, None, merged_opt.as_ref()).await {
        Ok(mut item) => {
            ...
            profiles_draft_update_item_safe(uid, &mut item).await?;
            handle::Handle::notice_message("update_with_system_proxy", profile_name);
            drop(last_err);
            return Ok(true);
        }
        Err(err) => {
    ...
    // Propagate the error so cmd/timer layers can surface a coded notice.
    // Do not swallow update failures as Ok(true).
    Err(last_err)
}
