...
async fn finalize_restored_verge_config(
    ...
    // Persist the restored clash file immediately; on OHOS this also rewrites
    // the sandbox-forced listen address / empty secret into the on-disk config.
    restored_clash.save_config().await?;
    ...
    if let Err(err) = super::patch_verge(&restored, true).await {
        logging!(error, Type::Backup, "应用恢复的 verge 配置失败: {err:#?}");
    }
    Ok(())
}
...
pub async fn create_backup_and_upload_webdav() -> Result<()> {
    let (file_name, temp_file_path) = backup::create_backup().await.map_err(|err| {
        logging!(error, Type::Backup, "创建备份失败: {err:#?}");
        err
    })?;
    ...
    if let Err(err) = backup::WebDavClient::global()
        ...
        .await
    {
        logging!(error, Type::Backup, "上传到 WebDAV 失败: {err:#?}");
        // 上传失败时重置客户端缓存
    ...
    if let Err(err) = temp_file_path.remove_if_exists().await {
        logging!(warn, Type::Backup, "删除临时文件失败: {err:#?}");
    }
...
pub async fn list_wevdav_backup() -> Result<Vec<ListFile>> {
    backup::WebDavClient::global().list().await.map_err(|err| {
        logging!(error, Type::Backup, "列出 WebDAV 备份文件失败: {err:#?}");
        err
    })
}
...
pub async fn delete_webdav_backup(filename: String) -> Result<()> {
    backup::WebDavClient::global().delete(filename).await.map_err(|err| {
        logging!(error, Type::Backup, "删除 WebDAV 备份文件失败: {err:#?}");
        err
    })
}
...
pub async fn restore_webdav_backup(filename: String) -> Result<()> {
    ...
    backup::WebDavClient::global()
        ...
        .await
        .map_err(|err| {
            logging!(error, Type::Backup, "下载 WebDAV 备份文件失败: {err:#?}");
            err
        })?;
...
{
    let (file_name, temp_file_path) = backup::create_backup().await.map_err(|err| {
        logging!(error, Type::Backup, "创建本地备份失败: {err:#?}");
        err
    })?;
    ...
    if let Err(err) = move_file(temp_file_path.clone(), target_path.clone()).await {
        logging!(error, Type::Backup, "移动本地备份文件失败: {err:#?}");
        // 清理临时文件
        if let Err(clean_err) = temp_file_path.remove_if_exists().await {
            logging!(warn, Type::Backup, "移动失败后删除临时备份文件失败: {clean_err:#?}");
        }
        return Err(err);
    }
    ...
    match fs::rename(&from, &to).await {
        ...
        Err(rename_err) => {
            ...
            logging!(
                warn,
                Type::Backup,
                "直接重命名备份文件失败，回退到复制/删除: {rename_err:#?}"
            );
            fs::copy(&from, &to)
                .await
...
pub async fn delete_local_backup(filename: String) -> Result<()> {
    ...
    if !target_path.exists() {
        logging!(warn, Type::Backup, "未找到本地备份文件: {}", filename);
        return Ok(());
    }
