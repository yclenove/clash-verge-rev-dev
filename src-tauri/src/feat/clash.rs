...
pub async fn restart_app() {
    ...
    #[cfg(target_env = "ohos")]
    {
        // OpenHarmony runtime has no process restart; bring the core back instead.
        logging!(info, Type::System, "OHOS 不支持进程重启，改为重启核心");
        handle::Handle::global().clear_is_exiting();
        restart_clash_core().await;
    }
...
fn after_change_clash_mode() {
    AsyncHandler::spawn(move || async {
        ...
        match mihomo.get_connections().await {
            ...
            Err(err) => {
                logging!(error, Type::Core, "获取连接失败: {err}");
            }
        }
    });
}
...
pub async fn change_clash_mode(mode: String) -> Result<(), String> {
    ...
    logging!(debug, Type::Core, "切换 Clash 模式为 {mode}");
    ...
    clash_data.save_config().await.map_err(|e| e.to_string())?;
    handle::Handle::refresh_clash();
...
pub async fn test_delay(url: String) -> anyhow::Result<u32> {
    ...
    let proxy_port = if proxy_enabled {
        Some(MixedPort::effective().await)
    } else {
        None
    };
