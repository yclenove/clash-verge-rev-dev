...
pub async fn toggle_system_proxy() -> bool {
    ...
    {
        logging!(error, Type::ProxyMode, "关闭全部连接失败: {err}");
    }
...
pub async fn copy_clash_env() {
    ...
    let export_text = match env_type {
        ...
        _ => {
            logging!(error, Type::ProxyMode, "copy_clash_env: 无效的环境变量类型! {env_type}");
            return;
        }
    };

    if clipboard.write_text(&export_text).is_err() {
        logging!(error, Type::ProxyMode, "写入剪贴板失败");
    }
}
