...
impl IProfiles {
    ...
    pub async fn reorder(&mut self, active_id: &String, over_id: &String) -> Result<()> {
        ...
        // Insert at new_idx in the post-removal array, matching dnd-kit arrayMove
        // semantics used by the frontend (forward drag lands after the target).
        let insert_idx = if new_idx > items.len() { items.len() } else { new_idx };
    ...
    pub async fn delete_item(&mut self, uid: &String) -> Result<(bool, Vec<String>)> {
        ...
        for delete_uid in delete_uids {
            let delete_uid_str = delete_uid.as_deref();
            // Only delete the chain item if no other profile still references it.
            let still_referenced = items.iter().any(|item| {
                item.option.as_ref().is_some_and(|op| {
                    op.merge.as_deref() == delete_uid_str
                        || op.script.as_deref() == delete_uid_str
                        || op.rules.as_deref() == delete_uid_str
                        || op.proxies.as_deref() == delete_uid_str
                        || op.groups.as_deref() == delete_uid_str
                })
            });
    ...
    /// Get remote/local subscriptions for tray (uid, name, whether base/current).
    /// Multi-sub merge is always on: every listed subscription is active; `is_current`
    /// only marks which profile provides the base proxy-groups / overrides.
    pub fn profiles_preview(&self) -> Option<Vec<IProfilePreview<'_>>> {
        self.items.as_ref().map(|items| {
            items
                .iter()
                .filter_map(|e| {
                    let itype = e.itype.as_deref()?;
                    if itype != "remote" && itype != "local" {
                        return None;
                    }
                    let (uid, name) = (e.uid.as_ref()?, e.name.as_ref()?);
                    let is_current = self.is_current_profile_index(uid);
                    Some(IProfilePreview { uid, name, is_current })
                })
                .collect()
        })
    }
