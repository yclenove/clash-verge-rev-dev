...
#[cfg(target_env = "ohos")]
fn sanitize_ohos_config(mut config: Mapping) -> Mapping {
    ...
    // TUN / redir / tproxy need privileges the OHOS sandbox does not grant.
    let mut tun = config
        .get("tun")
...
impl IClashTemp {
    pub async fn new() -> Self {
        ...
        match map_result {
            Ok(mut map) => {
                ...
                // 确保 secret 字段存在且不为空（OHOS 使用空 secret 对齐本地 API）
                ...
                #[cfg(target_env = "ohos")]
                {
                    Self(sanitize_ohos_config(Self::guard(map)))
                }
                #[cfg(not(target_env = "ohos"))]
                {
                    Self(Self::guard(map))
    ...
    pub fn template() -> Self {
        ...
        // resolve process only when needed (traffic ranking / connection details)
        map.insert("find-process-mode".into(), "strict".into());
        #[cfg(target_env = "ohos")]
        {
            Self(sanitize_ohos_config(map))
        }
        #[cfg(not(target_env = "ohos"))]
        {
            Self(map)
    ...
    pub async fn save_config(&self) -> Result<()> {
        #[cfg(target_env = "ohos")]
        {
            let config = sanitize_ohos_config(self.0.clone());
            help::save_yaml(&dirs::clash_path()?, &config, Some("# Generated by Clash Verge")).await
        }
        #[cfg(not(target_env = "ohos"))]
        {
