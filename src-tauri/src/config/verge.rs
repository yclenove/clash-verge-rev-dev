...
pub struct IVerge {
    ...
    /// 控制首页各个卡片的显示顺序
    #[serde(skip_serializing_if = "Option::is_none")]
    pub home_cards_order: Option<Vec<String>>,
    ...
    /// 持久化的链式代理节点列表（有序，元素为节点名字符串或完整代理定义对象）
    /// 配合 enhance 重建时自动恢复链式代理注入
    #[serde(skip_serializing_if = "Option::is_none")]
    pub proxy_chain_nodes: Option<serde_json::Value>,

    /// 链式代理注入的目标代理组名称
    #[serde(skip_serializing_if = "Option::is_none")]
    pub proxy_chain_group: Option<String>,
}
...
impl IVerge {
    ...
    pub fn patch_config(&mut self, patch: &Self) {
        ...
        patch!(home_cards);
        patch!(home_cards_order);
        patch!(enable_external_controller);
        patch!(proxy_chain_nodes);
        if patch.proxy_chain_group.is_some() {
            self.proxy_chain_group = patch.proxy_chain_group.clone();
        } else if let Some(nodes) = patch.proxy_chain_nodes.as_ref()
            && nodes.as_array().is_some_and(|arr| arr.is_empty())
        {
            // Frontend clears chain by sending empty proxy_chain_nodes. In that
            // case, clear group as well even when proxy_chain_group came as null
            // (serde Option cannot distinguish null vs omitted here).
            self.proxy_chain_group = None;
        }
    }
...
mod tests {
    ...
    #[test]
    fn patch_config_clears_chain_group_when_nodes_are_empty() {
        let mut current = IVerge {
            proxy_chain_group: Some("GLOBAL".into()),
            proxy_chain_nodes: Some(json!(["node-a", "node-b"])),
            ..IVerge::default()
        };

        let patch = IVerge {
            proxy_chain_nodes: Some(json!([])),
            proxy_chain_group: None,
            ..IVerge::default()
        };

        current.patch_config(&patch);
