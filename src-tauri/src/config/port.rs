...
impl Config {
    ...
    async fn resolve_startup_mixed_port_inner() -> Result<bool> {
        ...
        if verge.enable_random_port == Some(true) {
            ...
            {
                if candidate != selected_port {
                    Self::apply_startup_mixed_port_fallback(selected_port, candidate).await?;
                    logging!(
                        info,
                        Type::Setup,
                        "enable_random_port is on; mixed proxy port switched from {} to {}",
                        selected_port,
                        candidate
                    );
                    return Ok(true);
                ...
                logging!(
                    warn,
                    Type::Setup,
                    "enable_random_port is on but no available mixed proxy port was found; keeping configured port {}",
                    selected_port
                );
            }
        }
...
fn random_listener_port_seed() -> u16 {
    const SPAN: u32 = u16::MAX as u32 - MIN_FALLBACK_PORT as u32;
    let mut buf = [0u8; 2];
    let offset = if getrandom::fill(&mut buf).is_ok() {
        u16::from_le_bytes(buf) as u32 % SPAN
    } else {
        std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .map(|duration| duration.subsec_millis() % SPAN)
            .unwrap_or(0)
    };
    MIN_FALLBACK_PORT + offset as u16
}
