...
#[tauri::command]
pub async fn get_sys_proxy() -> CmdResult<Mapping> {
    use crate::core::sysopt_ohos::read_app_http_proxy;

    let mut map = Mapping::new();
    match read_app_http_proxy() {
        Ok(Some((host, port, bypass))) => {
            map.insert("enable".into(), true.into());
            map.insert("server".into(), format!("{host}:{port}").into());
            map.insert("bypass".into(), bypass.join(",").into());
        }
        _ => {
            map.insert("enable".into(), false.into());
            map.insert("server".into(), "".into());
            map.insert("bypass".into(), "".into());
        }
    }
    Ok(map)
}
