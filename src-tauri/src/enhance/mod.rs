...
async fn collect_profile_items() -> Result<ProfileItems> {
    ...
    let rules_uid_opt = current_item.current_rules().cloned();
    ...
    let rules_item_future = async {
        match rules_uid_opt {
            Some(uid) => {
                chain_item_or_default(profiles_arc.get_item(&uid).ok(), || ChainItem {
                    uid: "".into(),
                    data: ChainType::Rules(SeqMap::default()),
                })
                .await
    ...
    let (merge_item, script_item, rules_item, proxies_item, groups_item, global_merge, global_script, global_rules) = tokio::join!(
        ...
        chain_item_or_default(profiles_arc.get_item("Rules").ok(), || ChainItem {
            uid: "Rules".into(),
            data: ChainType::Rules(SeqMap::default()),
        },),
    );
...
fn process_seq_items(
    mut config: Mapping,
    _rules_item: ChainItem,
    proxies_item: ChainItem,
    groups_item: ChainItem,
) -> Mapping {
    // 每订阅 rules 已停用：规则统一走全局 Rules（enhance() 中的 global_rules）。
    // 保留参数以维持调用签名，收集到的每订阅 rules 不再应用。
...
fn merge_default_config(
    ...
    #[cfg(all(target_os = "linux", not(target_env = "ohos")))] tproxy_enabled: bool,
) -> Mapping {
    for (key, value) in clash_config.into_iter() {
        if key.as_str() == Some("tun") {
            ...
            if key.as_str() == Some("external-controller") {
                #[cfg(target_env = "ohos")]
                {
                    // OHOS uses HTTP API only (plugin listens on 127.0.0.1:9090).
                    let _ = (enable_external_controller, value);
                    config.insert(key, "127.0.0.1:9090".into());
                }
    ...
    if enable_dns_settings && let Ok(app_dir) = dirs::app_home_dir() {
            ...
            && let Ok(mut dns_config) = serde_yaml_ng::from_str::<serde_yaml_ng::Mapping>(&dns_yaml)
        {
            if crate::utils::dns_config::normalize_dns_listen(&mut dns_config) {
                logging!(warn, Type::Core, "migrated legacy Linux DNS listener to 127.0.0.1:1053");
            }
...
/// Merge proxy nodes from ALL other subscriptions into the current config.
///
/// **Limitation**: this reads each subscription's *raw* YAML file from disk,
/// so any per-profile enhance chains (merge / script / proxies / groups) that
/// the other subscriptions may have are NOT applied to the merged nodes.
/// Only the original proxy definitions are imported.
#[allow(clippy::cognitive_complexity)]
async fn merge_other_profiles(mut config: Mapping) -> Mapping {
    let profiles = Config::profiles().await;
    let profiles_arc = profiles.latest_arc();
    drop(profiles);

    let current_uid = profiles_arc.get_current().cloned();
    ...
    let profiles_dir = match dirs::app_profiles_dir() {
        Ok(d) => d,
        Err(_) => return config,
    };

    // 收集已有节点名称
    let mut existing_names: HashSet<std::string::String> = HashSet::new();
    if let Some(Value::Sequence(proxies)) = config.get(Value::from("proxies")) {
        for proxy in proxies {
            if let Value::Mapping(map) = proxy
                && let Some(Value::String(name)) = map.get(Value::from("name"))
            {
    ...
    let mut new_proxies: Vec<Value> = Vec::new();

    for item in &items {
        // 跳过当前订阅和非 remote/local 类型
        let itype = match item.itype.as_ref() {
            Some(t) => t.as_str(),
            None => continue,
        };
        ...
        if item.uid.as_ref() == current_uid.as_ref() {
            continue;
        }
        let file = match item.file.as_ref() {
            Some(f) => f,
            None => continue,
        };

        let path = profiles_dir.join(file.as_str());
        let mapping = match help::read_mapping(&path).await {
            Ok(m) => m,
            Err(_) => continue,
        };

        if let Some(Value::Sequence(proxies)) = mapping.get(Value::from("proxies")) {
            for proxy in proxies {
                if let Value::Mapping(map) = proxy
                    && let Some(Value::String(name)) = map.get(Value::from("name"))
                    && existing_names.insert(name.to_string())
    ...
    logging!(
        info,
        Type::Core,
        "multi-profile merge: adding {} proxies from other subscriptions",
        new_proxies.len()
    );

    // 收集新增节点名称
    let new_names: Vec<Value> = new_proxies
        .iter()
        .filter_map(|p| {
            if let Value::Mapping(map) = p {
                map.get(Value::from("name")).cloned()
            } else {
                None
            }
        })
    ...
    // 追加到 proxies
    if let Some(Value::Sequence(proxies)) = config.get_mut(Value::from("proxies")) {
...
pub async fn enhance() -> Result<(Mapping, HashSet<String>, HashMap<String, ResultLog>)> {
    ...
    // 全局规则覆盖（始终应用于所有订阅）。
    let config = if let ChainType::Rules(rules) = global_rules.data {
        use_seq(rules, config, "rules")
    } else {
        config
    };
    ...
    let config = {
        ...
        // Local-only HTTP API; empty secret matches the OHOS mihomo plugin defaults.
        config.insert("secret".into(), "".into());
        // TUN / redir / tproxy need privileges OHOS sandbox does not grant yet.
        let mut tun = config
            .get("tun")
