...
pub(crate) static CLASH_LOGGER: Lazy<Arc<ClashLogBuffer>> = Lazy::new(|| Arc::new(ClashLogBuffer::new()));
...
impl CoreManager {
    ...
    pub fn core_started(&self, mode: RunningMode) {
        if matches!(mode, RunningMode::Service)
            && let Some(store) = log_store::get()
        {
