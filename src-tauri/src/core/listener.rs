...
fn bind_claim(claim: &BindClaim) -> io::Result<Socket> {
    ...
    #[cfg(not(windows))]
    // A stopped TCP listener can leave TIME_WAIT entries; mihomo can safely reclaim those ports.
    socket.set_reuse_address(claim.transport == ListenerTransport::Tcp)?;
    #[cfg(windows)]
    {
...
mod tests {
    ...
    #[cfg(unix)]
    #[test]
    fn startup_scope_allows_a_stopped_listener_with_time_wait_connections() -> anyhow::Result<()> {
        let listener = TcpListener::bind((Ipv4Addr::LOCALHOST, 0))?;
        let port = listener.local_addr()?.port();
