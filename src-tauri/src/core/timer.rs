...
impl Timer {
    ...
    pub async fn init(&self) -> Result<()> {
        if self
            ...
            .is_err()
        {
            logging!(debug, Type::Timer, "定时器已初始化，跳过...");
            return Ok(());
        }
...

        {
            ...
            logging!(debug, Type::Timer, "已注册定时任务数量: {}", timer_map.len());
        ...
        if let Some(items) = Config::profiles().await.data_arc().get_items() {
            for item in items.iter() {
                ...
                {
                    logging!(info, Type::Timer, "立即执行已到期的定时任务: uid={}", uid);
                    let _ = self.command_tx.send(TimerCommand::RunNow(uid.clone()));
                }
            }
        }

        logging!(info, Type::Timer, "定时器初始化完成");
        Ok(())
    }
    ...
    pub async fn refresh(&self) -> Result<()> {
        ...
        if *cache == new_map {
            logging!(debug, Type::Timer, "定时任务无需变更");
            return Ok(());
        }

        logging!(info, Type::Timer, "正在刷新定时任务映射，数量: {}", new_map.len());
        *cache = new_map.clone();
        drop(cache);
    ...
    fn apply_timer_map(
        ...
        tasks.retain(|uid, state| {
            ...
            logging!(debug, Type::Timer, "已移除定时任务 uid={}", uid);
            false
        });
    ...
    fn mark_task_running(state: &mut TaskState, uid: &str, immediate: bool) -> bool {
        ...
        if immediate {
            logging!(debug, Type::Timer, "定时任务已在运行，跳过立即执行 uid={}", uid);
        } else {
            logging!(debug, Type::Timer, "定时任务已在运行，跳过 uid={}", uid);
        }
        false
    }
    ...
    fn spawn_update_task(uid: String, command_tx: mpsc::UnboundedSender<TimerCommand>) {
        logging!(info, Type::Timer, "开始定时任务: uid={}", uid);
        AsyncHandler::spawn(move || async move {
    ...
    async fn async_task(uid: &String) {
        ...
        logging!(debug, Type::Timer, "正在为配置运行定时任务: {}", uid);

        run_timer_profile_update_transition(
            || Self::emit_update_event(uid, true),
            || async {
                let _is_current = Config::profiles().await.latest_arc().current.as_ref() == Some(uid);
                logging!(debug, Type::Timer, "配置 {} 是否为当前激活配置: {}", uid, _is_current);

                feat::update_profile(uid, None, true, false, false).await
            },
            |result| match result {
                Ok(_) => {
                    logging!(
                ...
                Err(e) => {
                    ...
                    super::handle::Handle::notice_message("update_profile::error", format!("{e}"));
                }
            },
    ...
    async fn wait_until_resolve_done(max_wait: Duration) {
        let _ = timeout(max_wait, async {
            while !is_resolve_done() {
                logging!(debug, Type::Timer, "等待解析完成...");
