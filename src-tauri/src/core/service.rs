...
fn uninstall_service() -> Result<()> {
    logging!(info, Type::Service, "卸载服务");
...
fn install_service() -> Result<()> {
    ...
    logging!(info, Type::Service, "安装服务");
    ...
    if let Some((code, err)) = check_output_error(&output) {
        logging!(error, Type::Service, "安装服务失败，代码: {}，详情: {}", code, err);
...
fn uninstall_service() -> Result<()> {
    ...
    logging!(info, Type::Service, "卸载状态码: {}", status.code().unwrap_or(-1));
...
fn uninstall_service() -> Result<()> {
    ...
    // logging!(debug, Type::Service, "卸载命令: {}", command);
...
fn reinstall_service() -> Result<()> {
    logging!(info, Type::Service, "重新安装服务");
    install_service()
}
...
#[cfg(clash_verge_mobile)]
fn install_service() -> Result<()> {
    bail!("service mode is unsupported on Android")
}
...
async fn read_owner_sample() -> OwnerSample {
    ...
    let response = match response {
        Ok(response) => response,
        Err(error) => {
            logging!(debug, Type::Service, "服务所有者状态无法读取: {error:#}");
    ...
    let Some(status) = response.data else {
        logging!(debug, Type::Service, "服务所有者状态缺少数据");
        return OwnerSample::Unreadable;
    };
...
async fn recover_after_owner_loss_while_locked(reason: OwnerRecoveryReason) {
    logging!(
        warn,
        Type::Service,
        "服务所有者恢复 ({reason:?})；正在清除本地代理与 PAC 状态"
    );
    ...
    if let Some(error) = last_error {
        logging!(error, Type::Service, "失去所有者后清除本地代理失败: {error}");
    }
}
...
impl ServiceManager {
    ...
    async fn run_operation(&self, operation: impl Future<Output = Result<()>>) -> Result<()> {
        run_operation_and_then(&RUN_STATE, operation, || async {
            if let Err(error) = Tray::global().update_menu().await {
                logging!(warn, Type::Service, "服务操作后刷新托盘失败: {error:#}");
            }
            Ok(())
        })
        .await
    ...
    async fn apply_service_status(&self, status: ServiceStatus) -> Result<()> {
        ...
        logging!(info, Type::Service, "正在执行特权服务操作 {action:?}");
...
fn report_non_actionable_status(status: ServiceStatus) -> Result<()> {
    match status {
        ...
        ServiceStatus::NotInstalled => {
            logging!(info, Type::Service, "服务未安装；可使用 Sidecar");
        }
        ...
        ServiceStatus::SidecarAllowed => {
            logging!(info, Type::Service, "本次应用会话已明确允许使用 Sidecar");
        }
