...
impl Hotkey {
            ...
            HotkeyFunction::ReactivateProfiles => {
                AsyncHandler::spawn(async move || match feat::enhance_profiles().await {
                    ...
                    Ok(outcome) => {
                        ...
                        logging!(warn, Type::Hotkey, "快捷键重新激活配置验证失败: {}", message.as_str());
                        handle::Handle::notice_message("reactivate_profiles::error", message);
                    }
                    Err(err) => {
                        logging!(error, Type::Hotkey, "通过快捷键重新激活订阅失败: {}", err);
    ...
    pub async fn register_hotkey_with_function(&self, hotkey: &str, function: HotkeyFunction) -> Result<()> {
        ...
        logging!(debug, Type::Hotkey, "尝试注册快捷键: {}，功能: {}", hotkey, function);

        if manager.is_registered(hotkey) {
            logging!(debug, Type::Hotkey, "快捷键 {} 已注册，先取消注册", hotkey);
        ...
        manager.on_shortcut(hotkey, move |_app_handle, hotkey_event, event| match event.state {
            ...
            ShortcutState::Pressed => {
                if pressed.swap(true, Ordering::Relaxed) {
                    logging!(debug, Type::Hotkey, "忽略重复的快捷键按下: {:?}", hotkey_event);
                    return;
                }

                logging!(debug, Type::Hotkey, "快捷键已按下: {:?}", hotkey_event);
                let hotkey = hotkey_event.key;
                if hotkey == Code::KeyQ && is_quit {
                    if let Some(window) = WindowManager::get_main_window()
                        && window.is_focused().unwrap_or(false)
                    {
                        logging!(debug, Type::Hotkey, "正在执行退出功能");
                        Self::execute_function(function);
                    }
                } else {
                    AsyncHandler::spawn(move || async move {
                        logging!(debug, Type::Hotkey, "正在直接执行功能");
        ...
        logging!(debug, Type::Hotkey, "已成功注册快捷键 {}，功能 {}", hotkey, function);
        Ok(())
    }
}
...
impl Hotkey {
    pub async fn init(&self, skip: bool) -> Result<()> {
        ...
        if let Some(hotkeys) = hotkeys {
            ...
            for hotkey in hotkeys.iter() {
                ...
                match (key, func) {
                    (Some(key), Some(func)) => {
                        logging!(debug, Type::Hotkey, "正在注册快捷键: {} -> {}", key, func);
                        if let Err(e) = self.register(key, func).await {
                            logging!(error, Type::Hotkey, "注册快捷键失败 {} -> {}: {:?}", key, func, e);
                        } else {
                            logging!(debug, Type::Hotkey, "已成功注册快捷键 {} -> {}", key, func);
                        }
                    }
                    _ => {
                        let key = key.unwrap_or("None");
                        let func = func.unwrap_or("None");
                        logging!(error, Type::Hotkey, "无效的快捷键配置: `{}`:`{}`", key, func);
                    }
                }
            }
            self.current.store(Arc::new(hotkeys));
        } else {
            logging!(debug, Type::Hotkey, "未配置快捷键");
        }
    ...
    pub fn unregister(&self, hotkey: &str) -> Result<()> {
        ...
        let manager = app_handle.global_shortcut();
        manager.unregister(hotkey)?;
        logging!(debug, Type::Hotkey, "取消注册快捷键 {}", hotkey);
        Ok(())
    }
...
impl Drop for Hotkey {
    fn drop(&mut self) {
        ...
        if let Err(e) = app_handle.global_shortcut().unregister_all() {
            logging!(error, Type::Hotkey, "取消注册全部快捷键时出错: {:?}", e);
        }
    }
}
