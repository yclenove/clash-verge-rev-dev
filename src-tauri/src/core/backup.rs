...
impl WebDavClient {
    ...
    async fn get_client(&self, op: Operation) -> Result<reqwest_dav::Client, Error> {
        ...
        if let Err(e) = client.mkcol(dirs::BACKUP_DIR).await {
            ...
            if status_code == Some(409) {
                logging!(warn, Type::Backup, "无法创建备份目录，因为其父目录不存在");
                self.reset();
            ...
            if already_exists {
                logging!(info, Type::Backup, "备份目录已存在");
            } else {
                logging!(warn, Type::Backup, "创建备份目录失败: {}", e);
                self.reset();
                return Err(anyhow::Error::msg(format!("Failed to create backup directory: {}", e)));
            }
        } else {
            logging!(info, Type::Backup, "已成功创建备份目录");
        }
    ...
    pub async fn upload(&self, file_path: PathBuf, file_name: String) -> Result<(), Error> {
        ...
        .retry(backoff)
        .notify(|err, dur| {
            logging!(warn, Type::Backup, "上传失败: {err}，将在 {dur:?} 后重试");
        })
        .await
    }
